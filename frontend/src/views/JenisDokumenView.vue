...
const data = ref<JenisDokumen[]>([]);
const showModal = ref(false);
const showDeleteModal = ref(false);
const showBulkDeleteModal = ref(false);
const editingItem = ref<JenisDokumen | null>(null);
const deletingItem = ref<JenisDokumen | null>(null);
...
const openDelete = (item: JenisDokumen) => {
...
const save = async () => {
  ...
  try {
    if (editingItem.value) {
      await api.put(`/jenis-dokumen/${editingItem.value.id}`, form.value);
      toast.success("Data berhasil diupdate");
    } else {
      await api.post("/jenis-dokumen", form.value);
...
const confirmDelete = async () => {
  ...
  try {
    await api.delete(`/jenis-dokumen/${deletingItem.value.id}`);
...
const confirmBulkDelete = async () => {
  ...
  for (const id of selectedIds.value) {
    try {
      await api.delete(`/jenis-dokumen/${id}`);
      successCount++;
    } catch {
      failCount++;
    }
    ...
    >
      ...
      <template #actions="{ row }">
        <div class="flex gap-2">
          <button
            @click.stop="openEdit(row as JenisDokumen)"
            class="text-blue-600 hover:text-blue-800"
          >
            Edit
          </button>
          <button
            @click.stop="openDelete(row as JenisDokumen)"
